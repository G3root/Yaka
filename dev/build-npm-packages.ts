...
const reuseReleaseBinary = process.env.YAKA_REUSE_RELEASE_BINARY === "1"
...
const buildBinaryPackage = async (target: ReleaseTarget) => {
	...
	const binaryPath = join(binDir, "yaka")
	const releaseBinaryPath = join(root, "dist", "release", target.id, "yaka")
...
const buildMainPackage = async () => {
	...
	await cp(join(root, "bin", "yaka.js"), join(packageDir, "bin", "yaka.js"))
