...
type ThemeColor = Exclude<keyof ThemeColors, "thinkingOpacity">
...
let activeThemeName = "opencode"
let activeMode: "dark" | "light" = "dark"
let activeTheme: Theme = resolveTheme(opencode, activeMode)
...
export function resolveTheme(theme: ThemeJson, mode: "dark" | "light") {
	...
	if (hasSelectedListItemText) {
		...
		resolved.selectedListItemText = resolved.background ?? RGBA.fromInts(0, 0, 0)
	}
	...
	if (theme.theme.backgroundMenu !== undefined) {
		...
		resolved.backgroundMenu = resolved.backgroundElement ?? resolved.background ?? RGBA.fromInts(0, 0, 0)
	}
...
export const theme: Theme = new Proxy({} as Theme, {
	get(_target, prop) {
		return activeTheme[prop as keyof Theme]
	},
})

export const selectedTheme = () => activeThemeName

export const themeMode = () => activeMode

export function setActiveTheme(name: string, mode: "dark" | "light" = activeMode) {
	const themes = listThemes()
	const next = themes[name] ?? themes.opencode ?? opencode
	activeThemeName = themes[name] ? name : "opencode"
	activeMode = mode
	activeTheme = resolveTheme(next, mode)
	return themes[name] !== undefined
}

export function setSystemThemeColors(colors: TerminalColors, mode: "dark" | "light" = activeMode) {
	if (!colors.palette[0]) {
...
function refreshActiveTheme() {
	setActiveTheme(activeThemeName, activeMode)
}
